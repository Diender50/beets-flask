...
async def album_art(album_id: int, lib: BeetsLib, index: int = 0, size: str = "small") -> Response:
    ...
    if album.artpath and index == 0:
        ...
        # artpath set but file missing — fall through to other sources

    # Fallback: proxy Cover Art Archive image (index=0 only).
    # Must proxy (not redirect) to avoid CORS errors in the browser fetch() API.
    if index == 0:
        mb_release_id = getattr(album, "mb_albumid", None) or getattr(album, "mb_releaseid", None)
        if mb_release_id:
            import aiohttp
            caa_url = f"https://coverartarchive.org/release/{mb_release_id}/front-250"
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        ...
                        headers={"User-Agent": "beets-flask/2.0"},
                    ) as r:
                        if r.status == 200:
                            data = await r.read()
